  ...
  return (
    <nav
      className={
        isMobile
          ? dropdown
              ? 'navbar dropdown open flex justify-between h-16 items-center py-2 px-1 relative z-10' // Es móvil con dropdown abierto
              : 'navbar dropdown closed flex justify-between h-16 items-center py-2 px-1 relative z-10' // Es móvil con dropdown cerrado
          : 'flex justify-between h-16 items-center p-4  relative z-10 container'
      }
    >
      ...
      >
        ...
        >
          {!darkMode // SOL
            ? <><path d='M12 18C15.3137 18 18 15.3137 18 12C18 8.68629 15.3137 6 12 6C8.68629 6 6 8.68629 6 12C6 15.3137 8.68629 18 12 18Z' stroke='white' stroke-linecap='round' stroke-linejoin='round' />
      ...
      <ul
        className='m-0 list-none flex gap-4 min-h-0 w-auto ml-auto items-center z-10 rounded-2xl flex-col justify-center bg-primary-light-80 p-6 fixed bottom-32 top-auto opacity-100 transition-all duration-300 ease-in-out right-16 backdrop-blur backdrop-saturate-150 shadow-xl md:flex-row md:justify-between md:relative md:top-0 md:shadow-none md:backdrop-filter-none md:bg-transparent md:right-0 md:p-0 pb-2'
        ref={ref}
      >
        {
          ...
          Object.entries(menuItems).map(([menuLabel, menuLabelUrl], i) => {
            return (
              ...
              >
                <Btn
                  className='no-underline hover:underline hover:decoration-secondary-dark decoration-2 decoration-transparent underline-offset-2 duration-500 transition-all ease-in-out lg:ml-4 dark:text-darkest'
                  to={menuLabelUrl}
                >{menuLabel}
                </Btn>
      ...
      <button
        ...
        className={
          dropdown
            ? 'border-none bg-primary-light-80 m-0 cursor-pointer fixed right-16 bottom-16 z-10 flex p-3 rounded-full backdrop-blur-md shadow-lg transition-all duration-300 active md:invisible md:pointer-events-none'
            : 'border-none bg-secondary-light-80 m-0 cursor-pointer fixed right-16 bottom-16 z-10 flex p-3 rounded-full backdrop-blur-md shadow-lg transition-all duration-300 md:invisible md:pointer-events-none'
        }
      >
        <img
          className='h-9'
